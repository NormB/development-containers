...
if [ ! -f "./private/ca.key.pem" ]; then
  ...
  -subj \"/C=US/ST=New Jersey/O=GOES/OU=Certificates/CN=root.ca.goes.fake\" \
  -days 7300 -sha256 \
  ...
  eval "openssl req -config intermediate/openssl.cnf -new -sha256 \
    ...
    -subj \"/C=US/ST=New Jersey/O=GOES/OU=Certificates/CN=inter.ca.goes.fake\" \
...
if [ "$1" == "server" ]; then
  ...
  eval "openssl req -config intermediate/openssl.cnf \
    -subj \"/C=US/ST=New Jersey/O=GOES/OU=Servers/CN=$_DNS_NAME\" \
  ...
  eval "openssl req -config intermediate/openssl.cnf \
    -subj \"/C=US/ST=New Jersey/O=GOES/OU=Clients/CN=$_COMMON_NAME\" \
    -key \"$_PRIVATE_KEY\" -new -sha256 -out \"$_CSR\" $_OUTPUT"
